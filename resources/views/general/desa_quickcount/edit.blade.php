...
@section('content')
    <section class="p-3">
        <form action="{{ route('desa.quick_count.update', [$koorkota, $koorkecamatan, $koordesa, $quickcount]) }}"
            method="POST" enctype="multipart/form-data">
            @csrf

            <div class="mb-3">
                <label class="form-label">Nama TPS</label>
                <input type="text" name="name_tps" class="form-control" value="{{ $quickcount->name_tps }}">
            </div>
